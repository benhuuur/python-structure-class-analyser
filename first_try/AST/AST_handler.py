...
class AST_handler:
    ...
    def get_classes(tree_body):
        ...
        for node in tree_body:
            if isinstance(node, ast.ClassDef):
                ...
                assignments = list()
                assignments_data = AST_handler.get_assignments(node.body)
                for assignment, available in assignments_data:
    ...
    def get_assignments(node_body):
        ...
        for statement in node_body:
            if isinstance(statement, ast.Assign):
                assignments_data = AST_handler._get_assignment(statement)
                for assignment, available in zip(assignments_data[0], assignments_data[1]):
                    assignments.append((assignment, available))
    ...
    @staticmethod
    def _get_assignment(statement: ast.Assign):
        """
        ...
        assignment_names = AST_handler._get_assignment_names(
            statement.targets)

        assignment_isAttribute = AST_handler._check_types(
            statement.targets, ast.Attribute)
        ...
        # print("assignment_isAttribute", len(assignment_isAttribute))

        if (len(assignment_names) > 1):
            return ([assignments_info(name=assignment_names[i], data_type=assignment_data_type[i], encapsulation=assignment_encapsulation[i]) for i in range(len(assignment_names))], assignment_isAttribute)

        return ([assignments_info(name=assignment_names[0], data_type=assignment_data_type[0], encapsulation=assignment_encapsulation[0])], assignment_isAttribute)
    ...
    @staticmethod
    def _check_types(values: list, target_type):
        type_checks = []
        for value in values:
            type_checks.append(isinstance(value, target_type))
    ...
    def _get_init_assignment(node_body):
        ...
        for statement in node_body:
            if isinstance(statement, (ast.FunctionDef, ast.AsyncFunctionDef)) and statement.name == "__init__":
                assignments_data = AST_handler.get_assignments(statement.body)

                assignments = list()
                print("init", assignments_data)
                for assignment, available in assignments_data:
                    if available:
                        assignments.append(assignment)
